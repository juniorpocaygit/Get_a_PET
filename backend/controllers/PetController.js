...
module.exports = class PetController {
    ...
    static async removePetById(req,res){
        ...
        if(pet.user._id.toString() !== user._id.toString()){
            res.status(422).json({message: 'Houve um problema ao processar sua solicitção, tente novamente mais tarde.'})
            return 
        }
    ...
    static async schedule(req, res){
        ...
        if(pet.user._id.equals(user._id)){
            res.status(422).json({message: 'Você não pode agendar uma visita sua para o seu próprio pet!'})
            return 
        }
